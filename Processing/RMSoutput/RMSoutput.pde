...
void draw() {
  ...
  data=int(rms_scaled);//send to dmx
  println(rms_scaled);
  intensity=map(rms_scaled, 30, 200, 0, 255);
  smoothedData=lerp(smoothedData, intensity, 0.05);
  serialSend(); //send to Arduino
}

void serialSend() {
  //send the information out!
  myPort.write(int(smoothedData)+";");
}
